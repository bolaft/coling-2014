...
Automatic processing of online conversations (forum, emails) carrying information or assistance needs 
is a highly important issue for the industrial and the scientific communities which care to improve the present question/answering systems, identify the emotion or the intention in customer requests or reviews, detect messages containing requests for action, unsolved severe problems\ldots
...
%
A first critic of this paradigm is that it is not realistic to use it in the context of multi-domain and multimodal processing because it requires the building of training data which is a very substantial and time-consuming task.
...
A second critic is that the model does not have a fine-grained representation of the message structure or the relations between the messages. Considering such characteristics could drastically improve the systems for example to focus on specific text parts or to filter out the less relevant ones. 
...
This paper addresses the problem of message segmentation in online discussions. The process aims at supporting the analysis of messages in terms of dialogues acts.
...
The idea is to use the effort of the writer who responds to a message by inserting its response or comment just after the quoted text appropriate to his intervention. 
%
...
%
We assume that the quoted part will consist in an homogeneous information unit with respect to the context from which it is extracted. 
Consequently, we will make assumptions about the role played by the sentences in the organisation of the information in the source message.
For instance, we could assume that the first sentence of a quoted part carries instructions for opening a new discourse segment while the last sentence carries instructions for ending a segment.
