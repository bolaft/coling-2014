...
%
First, a quoted message is an altered version of the original one and the segmenter is intended to work on non-noisy data (i.e. the new content in messages). 
...
are not always standards-compliant and totally compatible\footnote{The \textit{Request for Comments} (RFC) are guidelines and protocols proposed by various working groups involved in the Internet Standardization \url{https://tools.ietf.org/html}. Some of the RFC are dedicated to email format and encoding specifications (See RFC 2822 and 5335 as starting points). The last 20 years, there were several propositions with updates and consequently obsoleted versions which may explain some of the alteration issues.}. 
In particular, the quoted parts can be wrongly recoded at each exchange step since there is no more dedicated header information about it.
%So each of them can 
...
%\itemsep0em 
%\item [$\bullet$ starting and ending] (\textit{SE}) a segment when there is only one sentence in the segment, 
%\item [$\bullet$ starting] (\textit{S}) a segment if there are at least two sentences in the segment and the sentence is the first one, 
%\item [$\bullet$ ending] (\textit{E}) a segment if there are at least two sentences in the segment and the sentence is the last one, 
%\item [$\bullet$ inside] (\textit{I}) a segment in any other cases.
%\end{description}
...
The generation procedure is intented to "automatically" annotating sentences from the source messages with segmentation information.
...
%\itemsep0em 
\item Identification of the messages with interleaved replies
\item For each pair of source and reply (with inline replying style) messages
\begin{enumerate}
...
\item Align the sentences from the source message with %the ones from 
the quoted text in the reply message 
...
%
The \textit{quoted lines} in the reply messages are identified based on the presence of a specific prefix at the beginning of the lines. 
As declared in the RFC~3676\footnote{\url{http://www.ietf.org/rfc/rfc3676.txt}}, we consider the lines of a message to be quoted if the first character is the sign "\texttt{>}" (greater than).
%
Lines which are not quoted lines are considered to be \textit{new content} lines.
%
\textit{Pairs of source and reply messages} are made up based on the \texttt{\footnotesize in-reply-to} field present in the email headers.
...
For labelling each aligned sentence with a segmentation instruction, we follow the following algorithm:
...
For each aligned source sentence %identified quoted sentence
 (sentence from the source message re-used in the reply message) \vspace{-0.1cm}
