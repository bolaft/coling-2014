...
For the purposes of analysis and experimentation, we distinguish four sets of features:  $n$-gram features, information structure based features, thematic features and stylistic features.
...
FIXME features are present in this set.
...
This feature set encompasses more than simply stylistic features. 
It includes graphic, orthographic and semantic features.
Somehow it consists in our catch-all category.
...
%\begin{itemize}
%	\item 
whether the sentence begins with or contains a ``wh*'' question word  (\textit{``who''}, \textit{``when''}, \textit{``where''}, \textit{``what''}, \textit{``which''}, \textit{``what''}, \textit{``how''}) or an n-gram suggesting an incoming interrogation (e.g. \textit{``is it''} or \textit{``are there''}); %, and whether the sentence merely contains such a word or n-gram
%	\item 
...
%	\item 
whether the sentence contains first person (such as \textit{``we''}, \textit{``my''} and \textit{``me''}), second person or third persons words; % (e.g. \textit{``we''}, \textit{``my''} and \textit{``me''} are recognized as first person words)
%	\item 
