...
%
Due to the size of the messages (and consequently the available lexical material), it is not always possible to exploit this kind of methods. Our results tend to indicate that we should explore more deeply this direction.
%
...
in our task, despite some noise, the compared text includes large parts of identical material from the source one. 
The kinds of edit operation in presence (no inversion\footnote{When computing the Levenshtein distance, the inversion edit operation is the most costly operation.} only deletion, insertion and substitution) leads us to consider the Levenshtein distance as a serious option.  
